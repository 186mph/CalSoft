...
const CALIBRATION_BLANKET_TABLE = 'calibration_blanket_reports' as const;
...
// Class options
const classOptions = ["0", "2", "3", "4"];

// Type options
const typeOptions = ["I", "II"];

// Split/Solid options
const splitSolidOptions = ["Solid", "Split"];

// Manufacturer options
const manufacturerOptions = ["Salisbury", "Novax", "Voltgard", "Chance", "Cementex", "White", "Hubble"];
...
interface BlanketTestHistory {
  id: string;
  blanket_report_id: string;
...
export default function CalibrationBlanketReport() {
  ...
  const handleBlanketDataChange = (field: keyof FormData['blanketData'], value: string) => {
    setFormData(prev => ({
      ...prev,
      blanketData: {
        ...prev.blanketData,
  ...
  // Update Pass/Fail status in the Blanket Data when overall status changes
  const updatePassFailStatus = (status: string) => {
    handleChange(null, 'status', status);
    handleBlanketDataChange('passFailStatus', status);
  };
  ...
  const autoSave = async (data: FormData) => {
    ...
    if (!reportId && (!data.blanketData.assetId || !data.customer)) return;
    ...
    try {
      ...
      console.log('Auto-saving blanket report...');
      ...
      if (reportId) {
        ...
        result = await supabase
          .schema(SCHEMA)
          .from(CALIBRATION_BLANKET_TABLE)
          .update(reportData)
          .eq('id', reportId)
          .select()
          .single();
        ...
        if (result.data) {
          ...
          const assetName = `Blanket Report - ${data.blanketData.manufacturer || ''} - ${new Date().toLocaleDateString()}`;
          const assetUrl = `report:/jobs/${jobId}/calibration-blanket/${result.data.id}`;
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      let assetIdToUse = formData.blanketData.assetId;
      ...
      if (reportId) {
        ...
        const { data, error } = await supabase
          .schema('lab_ops')
          .from('calibration_blanket_reports')
          .update({
      ...
      if (savedReportId && !reportId) {
        const assetName = `Blanket Report - ${formData.blanketData.manufacturer || ''} - ${new Date().toLocaleDateString()}`;
        const assetUrl = `report:/jobs/${jobId}/calibration-blanket/${savedReportId}`;
  ...
  const handlePrint = async () => {
    ...
    const options = {
      margin: 0.5,
      filename: `Blanket_Report_${formData.blanketData.assetId || 'New'}_${new Date().toISOString().split('T')[0]}.pdf`,
  ...
  const addTestHistoryEntry = async (testResult: 'PASS' | 'FAIL', notes?: string, reportIdToUse?: string) => {
    ...
    try {
      ...
      const { data, error } = await supabase
        ...
        .from('blanket_test_history')
        .insert({
          blanket_report_id: targetReportId,
  ...
  useEffect(() => {
    ...
    if (!reportId && formData.customerIdForAsset && !formData.blanketData.assetId) {
    ...
    <div className="p-4 max-w-7xl mx-auto space-y-8">
      ...
      <div className="flex justify-between items-center mb-6">
        <div className="flex items-center gap-2">
          ...
          <h1 className="text-2xl font-bold text-gray-900 dark:text-white">
            {reportId ? 'Edit Blanket Report' : 'New Blanket Report'}
          </h1>
        </div>
      ...
      <div ref={printRef} className="bg-white space-y-6 p-8 pb-16">
        ...
        <div className="bg-white dark:bg-dark-150 rounded-lg shadow p-6">
          <h2 className="text-xl font-semibold mb-4 text-gray-900 dark:text-white border-b dark:border-gray-700 pb-2">Blanket Information</h2>
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="block text-sm font-medium text-gray-700 dark:text-gray-300">Asset ID</label>
              <input 
                type="text" 
                value={formData.blanketData.assetId || "(Will be generated when saved)"}
                readOnly={true}
            ...
            <div>
              ...
              <select
                value={formData.blanketData.manufacturer}
                onChange={(e) => handleBlanketDataChange('manufacturer', e.target.value)}
                disabled={!isEditing}
            ...
            <div>
              ...
              <select
                value={formData.blanketData.class}
                onChange={(e) => handleBlanketDataChange('class', e.target.value)}
                disabled={!isEditing}
                className={`mt-1 block w-full rounded-md border-gray-300 dark:border-gray-700 bg-gray-50 dark:bg-dark-100 shadow-sm focus:border-accent-color focus:ring-accent-color text-gray-900 dark:text-white ${!isEditing ? 'bg-gray-100 dark:bg-dark-200' : ''}`}
              >
            ...
            <div>
              ...
              <select
                value={formData.blanketData.type}
                onChange={(e) => handleBlanketDataChange('type', e.target.value)}
                disabled={!isEditing}
                className={`mt-1 block w-full rounded-md border-gray-300 dark:border-gray-700 bg-gray-50 dark:bg-dark-100 shadow-sm focus:border-accent-color focus:ring-accent-color text-gray-900 dark:text-white ${!isEditing ? 'bg-gray-100 dark:bg-dark-200' : ''}`}
              >
            ...
            <div>
              <label className="block text-sm font-medium text-gray-700 dark:text-gray-300">Split/Solid</label>
              <select
                value={formData.blanketData.splitSolid}
                onChange={(e) => handleBlanketDataChange('splitSolid', e.target.value)}
                disabled={!isEditing}
                className={`mt-1 block w-full rounded-md border-gray-300 dark:border-gray-700 bg-gray-50 dark:bg-dark-100 shadow-sm focus:border-accent-color focus:ring-accent-color text-gray-900 dark:text-white ${!isEditing ? 'bg-gray-100 dark:bg-dark-200' : ''}`}
              >
                <option value="">Select Split/Solid</option>
                {splitSolidOptions.map(option => (
                  <option key={option} value={option}>{option}</option>
                ))}
              </select>
            </div>
              ...
              <div className="flex space-x-2 mt-1">
                <button
                  ...
                  className={`px-3 py-2 rounded-md text-white font-medium text-sm ${
                    formData.blanketData.passFailStatus === 'PASS' 
                ...
                <button
                  ...
                  disabled={!isEditing}
                  className={`px-3 py-2 rounded-md text-white font-medium text-sm ${
                    formData.blanketData.passFailStatus === 'FAIL' 
