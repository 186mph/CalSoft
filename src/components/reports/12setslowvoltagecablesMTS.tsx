...
// Main component
const TwelveSetsLowVoltageCableTestFormMTS: React.FC = () => {
  ...
  const loadReport = async () => {
    ...
    try {
      ...
      const { data, error } = await supabase
        .schema('neta_ops')
        .from('low_voltage_cable_test_12sets_mts')
        .select('*')
        .eq('id', reportId)
        .single();
  ...
  const handleSave = async () => {
    ...
    try {
        ...
        console.log('Attempting to save to schema: neta_ops, table: low_voltage_cable_test_12sets_mts');
        ...
        if (reportId) {
            ...
            try {
                ...
                const assetData = {
                    name: `3-Low Voltage Cable Test MTS (up to 12 sets) - ${formData.identifier || new Date().toLocaleDateString()}`,
                    file_url: `report:/jobs/${jobId}/low-voltage-cable-test-12sets-mts/${savedReportId}`,
                    user_id: user?.id,
                ...
                const { data: checkData, error: checkError } = await supabase
                    .from('neta_ops.low_voltage_cable_test_12sets_mts')
                    .select('id')
                    .limit(1);
                ...
                try {
                    ...
                    if (!fallbackError) {
                        alert("Fallback save successful. This indicates the low_voltage_cable_test_12sets_mts table has permission issues.");
  ...
  const renderHeader = () => (
    <div className="flex justify-between items-center mb-6">
      <h1 className="text-2xl font-bold text-gray-900 dark:text-white">
        3-Low Voltage Cable Test MTS (up to 12 sets)
      </h1>
...
export default TwelveSetsLowVoltageCableTestFormMTS; 