...
const CALIBRATION_HOTSTICK_TABLE = 'calibration_hotstick_reports' as const;
...
// Type options
const typeOptions = ["Rigid", "Shotgun", "Telescoping", "Tool", "Cutter", "Shepherd Hook"];

// Manufacturer options
const manufacturerOptions = ["Salisbury", "Hastings", "Chance", "Klein", "Blue Stripe", "PTA", "Lug All", "Lincoln Hoist", "Jenny Tools", "Nesco", "PENTA", "Utility Solutions", "Kearney", "Reliable", "Ward", "Crosby", "Pittsburg", "Little Mule", "Sling CO", "Jameson", "HD", "Armadillo", "NA"];
...
export default function CalibrationHotstickReport() {
  ...
  const handleHotstickDataChange = (field: keyof FormData['hotstickData'], value: string) => {
    setFormData(prev => ({
      ...prev,
      hotstickData: {
        ...prev.hotstickData,
  ...
  // Update Pass/Fail status in the Hotstick Data when overall status changes
  const updatePassFailStatus = (status: string) => {
    handleChange(null, 'status', status);
    handleHotstickDataChange('passFailStatus', status);
  };
  ...
  const loadReport = async () => {
    ...
    try {
      ...
      const { data: reportData, error: reportError } = await supabase
        .schema(SCHEMA)
        .from(CALIBRATION_HOTSTICK_TABLE)
        .select('*')
        .eq('id', reportId)
        .single();
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      let assetIdToUse = formData.hotstickData.assetId;
      ...
      if (reportId) {
        ...
        const { data, error } = await supabase
          .schema('lab_ops')
          .from('calibration_hotstick_reports')
          .update({
      ...
      if (savedReportId && !reportId) {
        const assetName = `Hotstick Report - ${formData.hotstickData.manufacturer || ''} - ${new Date().toLocaleDateString()}`;
        const assetUrl = `report:/jobs/${jobId}/calibration-hotstick/${savedReportId}`;
  ...
  const handlePrint = async () => {
      ...
      <div style="padding: 20px; font-family: Arial, sans-serif; color: black; background: white; font-size: 12px;">
        ...
        <div style="display: flex; margin-bottom: 15px;">
          ...
          <div style="flex: 1; margin-right: 20px;">
            ...
            <div style="margin-bottom: 8px;">${formData.hotstickData.assetId}</div>
            
            <div style="margin-bottom: 6px;"><strong>Manufacturer:</strong></div>
            <div style="margin-bottom: 8px;">${formData.hotstickData.manufacturer}</div>
            
            <div style="margin-bottom: 6px;"><strong>Model:</strong></div>
            <div style="margin-bottom: 8px;">${formData.hotstickData.model}</div>
          </div>
          ...
          <div style="flex: 1;">
            ...
            <div style="margin-bottom: 8px;">${formData.hotstickData.length}</div>
            
            <div style="margin-bottom: 6px;"><strong>Type:</strong></div>
            <div style="margin-bottom: 8px;">${formData.hotstickData.type}</div>
            
            <div style="margin-bottom: 6px;"><strong>Test Value:</strong></div>
            <div style="margin-bottom: 8px;">${formData.hotstickData.testValue}</div>
          </div>
        </div>
        ...
        <div style="margin-bottom: 20px;">
          ...
          <table style="width: 100%; border-collapse: collapse; font-size: 11px;">
            ...
            <tbody>
              <tr>
                <td style="padding: 4px; border: 1px solid #ccc;">Hastings 6799</td>
                <td style="padding: 4px; border: 1px solid #ccc;">06191708</td>
                <td style="padding: 4px; border: 1px solid #ccc;">5/7/2025</td>
                <td style="padding: 4px; border: 1px solid #ccc;">5/7/2026</td>
              </tr>
            </tbody>
          </table>
        </div>
    ...
    const options = {
      margin: 0.5,
      filename: `Hotstick_Report_${formData.hotstickData.assetId || 'New'}_${new Date().toISOString().split('T')[0]}.pdf`,
  ...
  useEffect(() => {
    ...
    if (!reportId && formData.customerIdForAsset && !formData.hotstickData.assetId) {
  ...
  return (
    <div className="p-4 max-w-7xl mx-auto space-y-8">
      ...
      <div className="flex justify-between items-center mb-6">
        <div className="flex items-center gap-2">
          ...
          <h1 className="text-2xl font-bold text-gray-900 dark:text-white">
            {reportId ? 'Edit Hotstick Report' : 'New Hotstick Report'}
          </h1>
        </div>
        <div className="flex items-center space-x-4">
          ...
          {reportId && !isEditing ? (
            ...
            <>
              <button
                onClick={() => navigate(`/jobs/${jobId}/calibration-hotstick`)}
                ...
                title="Create New Hotstick Report"
              >
      ...
      <div className="bg-white dark:bg-dark-150 rounded-lg shadow p-6">
        <h2 className="text-xl font-semibold mb-4 text-gray-900 dark:text-white border-b dark:border-gray-700 pb-2">Hotstick Information</h2>
        <div className="grid grid-cols-2 gap-4">
          <div>
            <label className="block text-sm font-medium text-gray-700 dark:text-gray-300">Asset ID</label>
            <input 
              type="text" 
              value={formData.hotstickData.assetId || "(Will be generated when saved)"}
              readOnly={true}
          ...
          <div>
            ...
            <input 
              type="text" 
              value={formData.hotstickData.model}
              onChange={(e) => handleHotstickDataChange('model', e.target.value)}
              disabled={!isEditing}
          ...
          <div>
            ...
            <select
              value={formData.hotstickData.manufacturer}
              onChange={(e) => handleHotstickDataChange('manufacturer', e.target.value)}
              disabled={!isEditing}
              className={`mt-1 block w-full rounded-md border-gray-300 dark:border-gray-700 bg-gray-50 dark:bg-dark-100 shadow-sm focus:border-accent-color focus:ring-accent-color text-gray-900 dark:text-white ${!isEditing ? 'bg-gray-100 dark:bg-dark-200' : ''}`}
            >
          ...
          <div>
            ...
            <input 
              type="text" 
              value={formData.hotstickData.length}
              onChange={(e) => handleHotstickDataChange('length', e.target.value)}
              disabled={!isEditing}
              className={`mt-1 block w-full rounded-md border-gray-300 dark:border-gray-700 bg-gray-50 dark:bg-dark-100 shadow-sm focus:border-accent-color focus:ring-accent-color text-gray-900 dark:text-white ${!isEditing ? 'bg-gray-100 dark:bg-dark-200' : ''}`}
            />
          </div>
          <div>
            ...
            <select
              value={formData.hotstickData.type}
              onChange={(e) => handleHotstickDataChange('type', e.target.value)}
              disabled={!isEditing}
              className={`mt-1 block w-full rounded-md border-gray-300 dark:border-gray-700 bg-gray-50 dark:bg-dark-100 shadow-sm focus:border-accent-color focus:ring-accent-color text-gray-900 dark:text-white ${!isEditing ? 'bg-gray-100 dark:bg-dark-200' : ''}`}
            >
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700 dark:text-gray-300">Test Value</label>
            <input 
              type="text" 
              value={formData.hotstickData.testValue}
              onChange={(e) => handleHotstickDataChange('testValue', e.target.value)}
              disabled={!isEditing}
              className={`mt-1 block w-full rounded-md border-gray-300 dark:border-gray-700 bg-gray-50 dark:bg-dark-100 shadow-sm focus:border-accent-color focus:ring-accent-color text-gray-900 dark:text-white ${!isEditing ? 'bg-gray-100 dark:bg-dark-200' : ''}`}
            />
          </div>
