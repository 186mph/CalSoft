...
const CALIBRATION_GROUND_CABLE_TABLE = 'calibration_ground_cable_reports' as const;
...
// Length options
const lengthOptions = ["25 ft", "50 ft", "75 ft", "100 ft", "150 ft", "200 ft"];

// Type options
const typeOptions = ["Temporary", "Permanent", "Portable"];

// Material options
const materialOptions = ["Copper", "Aluminum", "Steel"];

// Manufacturer options
const manufacturerOptions = ["Salisbury", "Novax", "Guardian", "Armadillo", "Marigold", "Magid", "Ansell", "DPL", "Chance", "White Rubber", "North"];

// Size options
const sizeOptions = ["1/0", "2/AWG", "2/0", "4/0"];
...
interface GroundCableTestHistory {
  id: string;
  ground_cable_report_id: string;
...
export default function CalibrationGroundCableReport() {
  ...
  const handleGroundCableDataChange = (field: keyof FormData['groundCableData'], value: string) => {
    setFormData(prev => ({
      ...prev,
      groundCableData: {
        ...prev.groundCableData,
        [field]: value
      }
    }));
  };
  ...
  // Update Pass/Fail status in the Ground Cable Data when overall status changes
  const updatePassFailStatus = (status: string) => {
    handleChange(null, 'status', status);
    handleGroundCableDataChange('passFailStatus', status);
  };
  ...
  const loadReport = async () => {
    ...
    try {
      ...
      const { data: reportData, error: reportError } = await supabase
        .schema(SCHEMA)
        .from(CALIBRATION_GROUND_CABLE_TABLE)
        .select('*')
        .eq('id', reportId)
        .single();
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      let assetIdToUse = formData.groundCableData.assetId;
      ...
      if (reportId) {
        ...
        const { data, error } = await supabase
          .schema('lab_ops')
          .from('calibration_ground_cable_reports')
          .update({
      ...
      if (savedReportId && !reportId) {
        const assetName = `Ground Cable Report - ${new Date().toLocaleDateString()}`;
        const assetUrl = `report:/jobs/${jobId}/calibration-ground-cable/${savedReportId}`;
  ...
  const handlePrint = async () => {
    ...
    const options = {
      margin: 0.5,
      filename: `Ground_Cable_Report_${formData.groundCableData.assetId || 'New'}_${new Date().toISOString().split('T')[0]}.pdf`,
  ...
  const loadTestHistory = async (reportId: string) => {
    ...
    try {
      ...
      const { data, error } = await supabase
        .schema('lab_ops')
        .from('ground_cable_test_history')
        .select(`
          id,
          test_date,
          test_result,
          ...
          created_at
        `)
        .eq('ground_cable_report_id', reportId)
  ...
  const addTestHistoryEntry = async (testResult: 'PASS' | 'FAIL', notes?: string, reportIdToUse?: string) => {
    ...
    try {
      ...
      const { data, error } = await supabase
        ...
        .from('ground_cable_test_history')
        .insert({
          ground_cable_report_id: targetReportId,
  ...
  useEffect(() => {
    ...
    if (!reportId && formData.customerIdForAsset && !formData.groundCableData.assetId) {
  ...
  return (
    <div className="p-4 max-w-7xl mx-auto space-y-8">
      ...
      <div className="flex justify-between items-center mb-6">
        <div className="flex items-center gap-2">
          ...
          <h1 className="text-2xl font-bold text-gray-900 dark:text-white">
            {reportId ? 'Edit Ground Cable Report' : 'New Ground Cable Report'}
          </h1>
        </div>
        <div className="flex items-center space-x-4">
          ...
          {reportId && !isEditing ? (
            ...
            <>
              <button
                onClick={() => navigate(`/jobs/${jobId}/calibration-ground-cable`)}
                className="bg-[#339C5E] hover:bg-[#2d8a52] text-white font-medium px-3 py-2 rounded-md flex items-center justify-center"
                title="Create New Ground Cable Report"
              >
      ...
      <div className="bg-white dark:bg-dark-150 rounded-lg shadow p-6">
        <h2 className="text-xl font-semibold mb-4 text-gray-900 dark:text-white border-b dark:border-gray-700 pb-2">Ground Cable Information</h2>
        <div className="grid grid-cols-2 gap-4">
          <div>
            <label className="block text-sm font-medium text-gray-700 dark:text-gray-300">Asset ID</label>
            <input 
              type="text" 
              value={formData.groundCableData.assetId || "(Will be generated when saved)"}
              readOnly={true}
              className={`mt-1 block w-full rounded-md border-gray-300 dark:border-gray-700 shadow-sm focus:border-accent-color focus:ring-accent-color cursor-not-allowed ${
                formData.groundCableData.assetId 
          ...
          <div>
            ...
            <input
              type="text"
              value={formData.groundCableData.length}
              onChange={(e) => handleGroundCableDataChange('length', e.target.value)}
              disabled={!isEditing}
          ...
          <div>
            ...
            <select
              value={formData.groundCableData.size}
              onChange={(e) => handleGroundCableDataChange('size', e.target.value)}
              disabled={!isEditing}
              className={`mt-1 block w-full rounded-md border-gray-300 dark:border-gray-700 bg-gray-50 dark:bg-dark-100 shadow-sm focus:border-accent-color focus:ring-accent-color text-gray-900 dark:text-white ${!isEditing ? 'bg-gray-100 dark:bg-dark-200' : ''}`}
            >
          ...
          <div>
            ...
            <input
              type="text"
              value={formData.groundCableData.resistance}
              onChange={(e) => handleGroundCableDataChange('resistance', e.target.value)}
              disabled={!isEditing}
              className={`mt-1 block w-full rounded-md border-gray-300 dark:border-gray-700 bg-gray-50 dark:bg-dark-100 shadow-sm focus:border-accent-color focus:ring-accent-color text-gray-900 dark:text-white ${!isEditing ? 'bg-gray-100 dark:bg-dark-200' : ''}`}
            />
          </div>
            ...
            <div className="flex space-x-2 mt-1">
              <button
                ...
                className={`px-3 py-2 rounded-md text-white font-medium text-sm ${
                  formData.groundCableData.passFailStatus === 'PASS' 
              ...
              <button
                ...
                disabled={!isEditing}
                className={`px-3 py-2 rounded-md text-white font-medium text-sm ${
                  formData.groundCableData.passFailStatus === 'FAIL' 
